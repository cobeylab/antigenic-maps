...
n_epitopes = 1
n_sera = 5
coords <- tibble(epitope = rep(c(1:n_epitopes), each = n_antigens+n_sera),
                         kind = rep(c('antigen', 'serum'), each = c(n_antigens, n_epitopes)),
                         id = c(1:n_antigens, 1:n_sera),
                         c1 = generate_ag_coords_random(n_antigens+n_sera, n_epitopes),
                         c2 = generate_ag_coords_random(n_antigens+n_sera, n_epitopes)) %>%
...
for(serum in 1:n_antigens){
  for(strain in 1:n_antigens){
    serum_coords = coords %>% filter(kind == 'serum') %>% slice(serum) %>% select(starts_with('c')) %>% as.vector()
    strain_coords = coords %>% filter(kind == 'antigen') %>% slice(strain) %>% select(starts_with('c')) %>% as.vector()
...
fit_stan_MDS <- function(
  ...
  list(sigma = 1,
       ag2_c1 = runif(1, 0, 10),
       antigen_coords = matrix(runif((n_antigens-2)*n_dim, 0, 10), n_antigens-2, n_dim),
       serum_coords = matrix(runif(n_sera*n_dim, 0, 10), n_sera, n_dim)
  )
}
...
```{r}
fits <- fit_stan_MDS(observed_distances = distances, n_antigens = n_antigens, n_sera = n_antigens, n_dim = n_dim, chains = 1, n_iter = 10000)
```
