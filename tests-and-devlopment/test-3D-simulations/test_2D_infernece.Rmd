---
title: "Test 2D simulations"
output: html_notebook
---

Check that when we simulate antigen coords in 2D with one epitope, that the 3D map recovers the underlying coords and distances.
...
```{r}
coords <- read_csv('../../experiments/ferret_immunodominance_tests/inputs/antigen_coords.csv')
coords
```


```{r}
inputs <- read_rds('../../experiments/ferret_immunodominance_tests/inputs/2D_E1_immunodominance_inputs.rds')
inputs
```
...
```{r}
## Fit
test_fit <- one_fit(n_dim = 2, titer_map = inputs$titer_map, n_chains = 4, n_iter = 7000, idstring = 'test')
```
...
compare_coords <- fit_summary %>% 
  ...
  merge(inputs$ag_ab_coords %>% select(antigen, serum, c1_Ag, c2_Ag) %>% distinct() %>%
...
```{r}
compare_coords %>%
  pivot_wider(names_from = 'coord', values_from = c('estimated_value', 'input_value')) %>%
  mutate(allele = as.factor(allele)) %>%
  ggplot() +
  geom_point(aes(x = input_value_1, y = input_value_2, color = allele, shape = kind)) +
  geom_point(aes(x = -estimated_value_1, y = estimated_value_2, color = allele), pch = 3) 
```
