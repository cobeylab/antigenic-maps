...
foreach(this_infection_year = unique(PCR_infections$`Infection Time`)) %do% {
  ...
  these_data %>%
    mutate(`Subject number` = as.factor(`Subject number`),
           ...
           xval_homologous_label = ifelse(is_homologous_strain, test_strain, NA),
            subject = sprintf('ID %i; YOB %i', `Subject number`, YOB),
  ...
  geom_segment(aes(x = as.numeric(test_strain)-.2, xend = as.numeric(test_strain)+.2, y = adjusted_distance, yend = adjusted_distance), color = 'gray')+
  ...
  filename = sprintf('../plots/HaNam_PCR_infections_%i.png', this_infection_year)
