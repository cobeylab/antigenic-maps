...
1. HaNam data from Fonville 2014

  1.a 12 PCR-confirmed infection in which the time since infection is known. Time between infection and sample ranges from 21-303 days.
  1.b 35 seroconversions in which the year of seroconversion is known, but the date of infection is not.
...
## Read in data on PCR-confirmed infections
```{r include=FALSE}
setwd('../../Ha_Nam/R/')
...
pcr_infections %>%
 ...
 # geom_point(aes(x = 1970, y = 6, color = is_infection_year))+
  ...
  ylab('logtiter')
ggsave('../plots/pcr_landscapes.png', width = 9, height = 12, units = 'in')
```

The above figure is similar to antigenic landscapes from Fonville 2014, with the following changes: The x-axis shows the year of strain circulation, and the y axis shows the log gmt to all strains that circulated in the year of interest. The values are not smoothed. THe top left shows the days elapsed from infection to sample collection. The blue upward and pink downward arrows show changes in titer from the previous annual sample.
