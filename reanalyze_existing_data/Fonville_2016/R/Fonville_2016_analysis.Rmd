...
## Replot without annotation for build slide
```{r}
individual_distances %>%
   mutate(undetectable_titers = ifelse(logtiter<0, logtiter, NA)) %>%
   mutate(ribbon_vals = ifelse(circulated_in_infection_window, strain_year, NA)) %>%
...
merged_distances %>%
 ...
 # geom_segment(aes(x = x_dodge-.2, xend = x_dodge+.2, y = expected_logtiter_adjusted_distance, yend = expected_logtiter_adjusted_distance), color = 'deepskyblue') +
  ...
  facet_wrap(.~subject_label) +
  ylab('log2 titer')
ggsave('../../draft_manuscript_figures/plots/primary_representative_indiv.png', width = 4, height = 3, dpi = 300, units = 'in')
```
...
titer_summary  %>% select(id, kind, max_logtiter, n_detectable, steepness) %>%
  ...
  pivot_longer(3:5, names_to = 'statistic', values_to = 'value') %>%
  ggplot() +
  geom_histogram(aes(x = value, fill = kind), alpha = .5, position = 'dodge', binwidth = 1) +
 # geom_point(aes(x = jitter(as.numeric(as.factor(kind))), y = value, color = id), show.legend = F)+
...
titer_summary  %>% select(id, kind, max_logtiter, n_detectable, steepness) %>%
  ...
  pivot_longer(3:5, names_to = 'statistic', values_to = 'value') %>%
  group_by(kind) %>%
  mutate(n_kind = length(unique(id))) %>%
  ungroup() %>%
  mutate(statistic = factor(statistic, levels = c('max_logtiter', 'n_detectable', 'steepness'), labels = c('height','width','steepness')),
           kind = factor(kind, labels = unique(sprintf('%s (n=%i)', kind, n_kind)))) %>%
  ggplot() +
  geom_boxplot(aes(x = statistic, y = value, fill = kind), alpha = .5) +
  xlab('')+
  theme(legend.position = c(.8, .8))
ggsave('../plots/ferret_child_titer_boxplots.png', width = 7.5, height = 2.5, units = 'in', dpi = 200)
```
...
merged_byyear = bind_rows(ferret_byyear,
                          ...
                          vaccine_1997,
                          PCR_infections) %>%
  mutate(kind_combined = kind,
         kind = sprintf('%s distances: %s strains', kind, ifelse(dy<0, 'past', 'future')),
         pf = ifelse(dy<0, 'past', 'future')) 
```


## Replicate the individual-ferret comparison plots from above
```{r}
## PCR-confirmed infections
read_csv('../../Ha_Nam/processed_data/PCR_infections.csv') %>%
  filter(`Infection Time` == `Sample year`) %>%  
  mutate(logtiter = ifelse(logtiter<0, NA, logtiter),
         homologous_titer = ifelse(logtiter<0, NA, homologous_titer),
         ferret_map_distance = ifelse(logtiter<0, NA, ferret_distance)) %>%
  mutate(expected_logtiter_ferret_distance = homologous_titer - ferret_distance,
         is_undetectable = ifelse(logtiter<0, logtiter, NA),
         subject_label = sprintf('%s, YOB:%s, S:%s', `Subject number`, YOB, `Sample year`)) %>%
  merge(read_csv('../../Ha_Nam/processed_data/HaNam_longshort_strain_names.csv') %>%
          rename(test_strain = long_name),
        all_x = T) %>%
  mutate(test_strain = factor(test_strain, levels = get_strains_chronologically(test_strain, test_strain_year), labels = get_strains_chronologically(short_name, test_strain_year))) %>%
  mutate(x_coord = as.numeric(test_strain),
         ribbon_vals = ifelse(is_homologous_strain, as.numeric(as.factor(x_coord)), NA)) %>%
  ## Summarize so that there is one homologous titer per individual
  group_by(subject_label, test_strain, x_coord) %>%
  summarise(logtiter = unique(logtiter),
            homologous_titer = mean(homologous_titer),
            ferret_distance = mean(ferret_map_distance),
            expected_logtiter_ferret_distance = homologous_titer - ferret_distance,
            is_homologous_strain = unique(is_homologous_strain),
            ribbon_vals = unique(ribbon_vals)) %>%
  ggplot() +
  geom_point(aes(x = test_strain, y = logtiter, group = subject_label), color = NA) +
  geom_ribbon(aes(x = x_coord, ymin = -10, ymax = 0), fill = 'gray', alpha = .5) +
  geom_ribbon(aes(x = ribbon_vals, ymin = -10, ymax = 8), fill = 'aquamarine', alpha = .5) +
  geom_vline(aes(xintercept = ribbon_vals), lty = 2, color = 'aquamarine')+
  geom_hline(aes(yintercept = 0), lty = 3)+
  geom_hline(aes(yintercept = homologous_titer), lty = 2)+
  geom_vline(aes(xintercept = is_homologous_strain), lty = 2, color = 'springgreen3')+
  geom_segment(aes(x = x_coord, xend = x_coord, y = logtiter, yend = expected_logtiter_ferret_distance), color = 'firebrick2', lty = 2) +
  geom_segment(aes(x = x_coord, xend = x_coord, y = homologous_titer, yend = logtiter, lty = logtiter<0), show.legend = F) +
  geom_point(aes(x = x_coord, y = logtiter)) +
  geom_segment(aes(x = x_coord-.2, xend = x_coord+.2, y = expected_logtiter_ferret_distance, yend = expected_logtiter_ferret_distance),  color = 'firebrick2') +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  facet_wrap(.~subject_label) +
  ylab('log2 titer') +
  xlab('strain')
ggsave('../plots/secondary_infection_comparison.png', width = 9, height = 6, dpi = 300, units = 'in')
```
...
read_csv('../../Ha_Nam/processed_data/PCR_infections.csv') %>%
  ...
  ggplot() +
  geom_point(aes(x = test_strain, y = logtiter, group = subject_label), color = NA) +
  geom_ribbon(aes(x = x_coord, ymin = -15, ymax = 0), fill = 'gray', alpha = .5) +
  geom_ribbon(aes(x = ribbon_vals, ymin = -15, ymax = 10), fill = 'aquamarine', alpha = .5) +
  geom_vline(aes(xintercept = ribbon_vals), lty = 2, color = 'aquamarine')+
  geom_hline(aes(yintercept = 0), lty = 3)+
  geom_hline(aes(yintercept = homologous_titer), lty = 2)+
  geom_vline(aes(xintercept = is_homologous_strain), lty = 2, color = 'springgreen3')+
  ...
  ylab('log2 titer') +
  xlab('strain')
ggsave('../../draft_manuscript_figures/plots/secondary_representative_indiv.png', width = 4, height = 3, dpi = 300, units = 'in')
```
...
```{r}
formula_vec = c('distance ~ ferret_distance', 
                'distance ~ ferret_distance + lag ',
                'distance ~ ferret_distance*pf', # pf stands for past/future
                'distance ~ ferret_distance*pf + lag', # separate slopes for past/future
                'distance ~ ferret_distance + lag + pf' ) # separate intercepts for past_future

## Run all fits
child_fit_list_past = lapply(formula_vec, one_lm, df = merged_byyear %>% filter(kind_combined == 'child')) 
secondary_infection_fit_list = lapply(formula_vec, one_lm, df = merged_byyear %>% filter(kind_combined == 'HaNam PCR infection'))
vaccine_fit_list = lapply(formula_vec[c(1,3)], one_lm, df = merged_byyear %>% filter(kind_combined == 'vaccine study')) 
```

## Del AIC
```{r}
summarize_fits = function(fit_list){
  lapply(fit_list, function(ll) ll$summary)%>%
  bind_rows() %>%
  arrange(AIC) %>%
...
```{r}
child_fit_table = summarize_fits(child_fit_list)
child_fit_table
```

```{r}
secondary_fit_table = summarize_fits(secondary_infection_fit_list)
secondary_fit_table
```

```{r}
vaccine_fit_table = summarize_fits(vaccine_fit_list)
vaccine_fit_table
```
## F test 
```{r}
...
## Repeat, fitting separately to past/future strains
```{r}
formula_vec = c('distance ~ ferret_distance', 
                'distance ~ ferret_distance + lag ') # separate slopes for past/future

## Run all fits
kinds = unique(merged_byyear$kind)[c(3,4,7,8)]
kinds

child_future_fit_list =  lapply(formula_vec, one_lm, df = merged_byyear %>% filter(kind == kinds[1]))
child_past_fit_list =  lapply(formula_vec, one_lm, df = merged_byyear %>% filter(kind == kinds[2]))
secondary_future_fit_list = lapply(formula_vec, one_lm, df = merged_byyear %>% filter(kind == kinds[4]))
secondary_past_fit_list = lapply(formula_vec, one_lm, df = merged_byyear %>% filter(kind == kinds[3]))
```

```{r}
summarize_fits(child_future_fit_list)
summarize_fits(child_past_fit_list)
summarize_fits(secondary_past_fit_list)
summarize_fits(secondary_future_fit_list)
```
...
```{r}
# ## Simple regression of distance by ferret distance
...
#          slope_lower = slope-1.96*summary(this_fit)$coefficients[2,2],
#          slope_upper = slope+1.96*summary(this_fit)$coefficients[2,2])
# }
# 
...
## Return predictions of each fit:
```{r}
## Simple regression of distance by ferret distance
merged_fits_simple = lapply(unique(merged_byyear$kind)[-c(1,2)], function(kk){
  this_df = filter(merged_byyear, kind == kk) %>%
    distinct()
  this_fit = lm(distance ~ ferret_distance, data = this_df)
  this_prediction = predict.lm(this_fit, se.fit = TRUE, newdata = this_df)
  this_df %>%
    ungroup() %>%
    mutate(prediction = this_prediction$fit,
           pred_low = this_prediction$fit - 1.96*this_prediction$se.fit,
           pred_high = this_prediction$fit + 1.96*this_prediction$se.fit,
           slope = this_fit$coefficients[2],
         intercept = this_fit$coefficients[1],
         rsquared = summary(this_fit)$r.squared,
         slope_pval = summary(this_fit)$coefficients[2,4],
...
## relabel merged_fits_simple
```{r}
merged_fits_simple = merged_fits_simple %>%
  separate(kind, into = c( 'distance type', 'past/future'), sep = ":", remove = F) %>%
  mutate(`past/future` = factor(`past/future`, levels = c(' past strains',' future strains')),
         `distance type` = factor(`distance type`, 
                                  levels = c('ferret distances','child distances', 
                                             'vaccine study distances', 
                                             'HaNam PCR infection distances'),
                                  labels = c('ferret distances',
                                             'primary infection', 
...
comparison_plot_distance= merged_fits_simple %>%
  filter(!grepl('ferret', kind)) %>%
  mutate(labs = factor(kind, labels = c('B.', 'A.', 'F.', 'E.', 'D.', 'C.'))) %>%
  ggplot() +
  geom_point(aes(x = jitter(ferret_distance), y = jitter(distance), color = lag), show.legend = F, alpha = .7)+
  geom_line(aes(x = ferret_distance, y = prediction)) +
  geom_ribbon(aes(x = ferret_distance, ymin = pred_low, ymax = pred_high), alpha = .3) +
  geom_text(aes(x = lab_x, y = lab_y, label = sprintf('%2.2f%s\nR2=%2.2f', slope, slope_pval_star, rsquared)), hjust = 0)+
  geom_hline(aes(yintercept=0),lty=3)+
  ...
  facet_grid(`distance type`~`past/future`, scales = 'free_x')+
  #geom_text(aes(x = ifelse(`past/future` == ' past strains', -35, 1), y = 9, label = labs))+
  ylab('individualdistance')+
  scale_color_manual(values = c('goldenrod', 'cornflowerblue', 'gray'))+
  #scale_color_viridis_d()+
  ylim(c(-8, 10))
comparison_plot_distance
ggsave(plot = comparison_plot_distance, '../plots/distances_by_distance_child_ferret_adult.png', width = 9, height = 9, dpi = 250, units = 'in')
```
...
comparison_plot_distance= merged_fits_simple %>%
  ...
  mutate(labs = factor(kind, labels = c('B.', 'A.', 'F.', 'E.', 'D.', 'C.'))) %>%
  ggplot() +
  geom_point(aes(x = jitter(ferret_distance), y = jitter(distance)), show.legend = F, alpha = .7)+
  geom_line(aes(x = ferret_distance, y = prediction)) +
  geom_ribbon(aes(x = ferret_distance, ymin = prediction-2, ymax = prediction+2), alpha = .3) +
  #geom_text(aes(x = lab_x, y = lab_y, label = sprintf('%2.2f%s\nR2=%2.2f', slope, slope_pval_star, rsquared)), hjust = 0)+
  geom_hline(aes(yintercept=0),lty=3)+
  geom_blank(aes(x =value, y = 1), data = x_lims)+
  #geom_abline(aes(intercept = 0, slope = ifelse(`past/future`==' past strains', -1, 1)), lty = 2) +
 ...
 # scale_color_manual(values = c('goldenrod', 'cornflowerblue', 'gray'))+
  ...
  ylim(c(-8, 10))
comparison_plot_distance
ggsave(plot = comparison_plot_distance, '../plots/distances_by_distance_child_ferret_adult_4fold.png', width = 9, height = 9, dpi = 250, units = 'in')
```
...
```{r}
merged_fits_simple %>%
  mutate(prediction_error = distance - prediction) %>% 
  ggplot() +
  geom_histogram(aes(x = prediction_error, fill = lag)) +
  facet_grid(`distance type`~`past/future`, scales = 'free_y')+
  scale_fill_manual(values = c('goldenrod', 'cornflowerblue', 'gray'))+
  geom_vline(aes(xintercept = c(-2)), lty = 2)+
  geom_vline(aes(xintercept = c(2)), lty = 2)
```
```{r}
merged_fits_simple %>%
  mutate(prediction_error = distance - prediction) %>% 
 # filter(!grepl(x = kind, pattern = 'vaccine')) %>%
  ggplot() +
  geom_histogram(aes(x = prediction_error, fill = id), show.legend = F) +
  facet_grid(`distance type`~`past/future`, scales = 'free_y')+
  geom_vline(aes(xintercept = c(-2)), lty = 2)+
  geom_vline(aes(xintercept = c(2)), lty = 2)
```
...
merged_fits_simple %>%
  group_by(kind, id) %>%
  summarise(frac_lost_cross_reactivity = sum((distance-prediction)>=2, na.rm = T)/n()) %>%
  ungroup()
```

## Highlight indivs that lost
```{r}
comparison_plot_distance=merged_fits_simple %>%
  filter(!grepl('ferret', kind)) %>%
  mutate(special_id = 'T') %>%
  ggplot() +
  geom_point(aes(x = jitter(ferret_distance), y = jitter(distance), color = special_id), show.legend = F, alpha = .7)+
  #  geom_point(aes(x = jitter(ferret_distance), y = jitter(distance)), show.legend = F, alpha = .7, color = 'red', data = merged_fits_simple %>% filter(id == 'S04-3'))+
  geom_line(aes(x = ferret_distance, y = prediction)) +
  geom_ribbon(aes(x = ferret_distance, ymin = prediction-2, ymax = prediction+2), alpha = .3) +
 # geom_text(aes(x = lab_x, y = lab_y, label = sprintf('%2.2f%s\nR2=%2.2f', slope, slope_pval_star, rsquared)), hjust = 0)+
  geom_hline(aes(yintercept=0),lty=3)+
 ...
 # scale_color_manual(values = c('goldenrod', 'cornflowerblue', 'gray'))+
  scale_color_viridis_d()+
  ylim(c(-8, 10))
ggsave(plot = comparison_plot_distance, '../plots/distances_by_distance_child_ferret_specialID_1to1.png', width = 9, height = 9, dpi = 250, units = 'in')
```
...
## Coefficient by type
```{r}
slope_plot = lm_fits_table_by_ferretdistance %>%
  mutate(label = c('primary: future', 'primary: past', 'secondary: future', 'secondary: past', 'secondary: past', 'secondary: future'),
         kind = factor(kind, labels = c('primary infection:\nfuture strains',
                                        'primary infection:\npast strains',
                                        'secondary vaccination:\nfuture strains',
                                        'secondary vaccination:\npast strains',
                                        'secondary infection:\npast strains',
                                        'secondary infection:\nfuture strains'))) %>%
  ggplot() +
  geom_point(aes(x = abs(slope), y = kind, color = label)) +
  geom_segment(aes(x = abs(slope_lower), xend = abs(slope_upper), y = kind, yend = kind, color = label)) +
  scale_color_manual(values = c('dodgerblue', 'blue4', 'orange', 'red3')) +
  ylab('')+xlab('slope') +
  theme(legend.position = 'below')

R2plot = lm_fits_table_by_ferretdistance %>%
  mutate(label = c('primary: future', 'primary: past', 'secondary: future', 'secondary: past', 'secondary: past', 'secondary: future'),
         kind = factor(kind, labels = c('primary infection:\nfuture strains',
                                        'primary infection:\npast strains',
                                        'secondary vaccination:\nfuture strains',
                                        ...
                                        'secondary infection:\nfuture strains'))) %>%
  ggplot() +
  geom_bar(aes(x = rsquared, y = kind, color = label, fill = label), stat = 'identity') +
  scale_color_manual(values = c('dodgerblue', 'blue4', 'orange', 'red3'), aesthetics = c('color', 'fill')) +
  ylab('')+xlab('R squared') +
  theme(legend.position = 'below')

frac_negative = merged_byyear %>%
  group_by(kind) %>%
  filter(!is.na(distance)) %>%
  filter(!grepl('ferret', kind)) %>%
  summarise(n_negative = sum(distance<0),
            n = n(),
            frac_negative = n_negative/n) %>%
  rowwise() %>%
  mutate(lower = binom::binom.confint(n_negative, n)[11, 5],
         upper = binom::binom.confint(n_negative, n)[11, 6]) %>%
  ungroup() %>%
  mutate(kind = factor(kind, labels = c('primary infection:\nfuture strains',
                                        'primary infection:\npast strains',
                                        'secondary vaccination:\nfuture strains',
                                        'secondary vaccination:\npast strains',
                                        'secondary infection:\npast strains',
                                        'secondary infection:\nfuture strains')),
         label = c('primary: future', 'primary: past', 'secondary: future', 'secondary: past', 'secondary: past', 'secondary: future')) %>%
  ggplot()+
  geom_point(aes(x = frac_negative, y = kind, color = label)) +
  geom_segment(aes(x = abs(lower), xend = abs(upper), y = kind, yend = kind, color = label)) +
  scale_color_manual(values = c('dodgerblue', 'blue4', 'orange', 'red3')) +
  ylab('')+xlab('frac. individual distances <0') +
  theme(legend.position = 'below')

cowplot::plot_grid(slope_plot, R2plot, frac_negative, ggplot()+geom_blank(),
                   nrow = 2, labels = list('G', 'H', 'I', 'J'))
```
...
comparison_plot_distance = merged_byyear %>%
  ...
  merge(lm_fits_table_by_ferretdistance) %>%
    mutate(ferret_distance_binned = factor(floor(ferret_distance), levels = sort(unique(floor(ferret_distance))), labels = abs(sort(unique(floor(ferret_distance))))),
...
#  geom_blank(aes(x =value+36, y = 1), data = x_lims)+
 ...
 # scale_color_manual(values = c('salmon', 'mediumturquoise', 'goldenrod', 'yellow3', 'red', 'slateblue4',  'red', 'dodgerblue')) +
  ...
  theme_classic() 
comparison_plot_distance
ggsave(plot = comparison_plot_distance, '../plots/distance_violins.png', width = 12, height = 6, dpi = 250, units = 'in')
```

```{r}
past_violins = merged_byyear %>%
  filter(!grepl('ferret', kind)) %>%
  separate(kind, into = c( 'distance type', 'past/future'), sep = ":", remove = F) %>%
  filter(`past/future` == ' past strains') %>%
  mutate(`past/future` = factor(`past/future`, levels = c(' past strains',' future strains')),
         `distance type` = factor(`distance type`, levels = c('ferret distances','child distances', 'vaccine study distances', 'HaNam PCR infection distances'), labels = c('ferrets','primary infections', 'secondary vaccinations', 'secondary infections'),  ordered = TRUE),
         ferret_distance = ifelse(`past/future` == ' past strains', -ferret_distance, ferret_distance)) %>%
  merge(lm_fits_table_by_ferretdistance) %>%
    mutate(ferret_distance_binned = factor(floor(ferret_distance), levels = sort(unique(floor(ferret_distance))), labels = abs(sort(unique(floor(ferret_distance))))),
           nbin = as.numeric(ferret_distance_binned)) %>%
...
future_violins = merged_byyear %>%
  filter(!grepl('ferret', kind)) %>%
  separate(kind, into = c( 'distance type', 'past/future'), sep = ":", remove = F) %>%
  filter(`past/future` == ' future strains') %>%
  mutate(`past/future` = factor(`past/future`, levels = c(' past strains',' future strains')),
         `distance type` = factor(`distance type`, levels = c('ferret distances','child distances', 'vaccine study distances', 'HaNam PCR infection distances'), labels = c('ferrets','primary infections', 'secondary vaccinations', 'secondary infections'),  ordered = TRUE),
         ferret_distance = ifelse(`past/future` == ' past strains', -ferret_distance, ferret_distance)) %>%
  merge(lm_fits_table_by_ferretdistance) %>%
...
cowplot::plot_grid(past_violins,
                   future_violins,
                   ncol = 2, rel_widths = c(1, .5))


ggsave('../plots/distance_violins.png', width = 12, height = 6, dpi = 250, units = 'in')
ggsave('../../draft_manuscript_figures/distance_violins.png', width = 12, height = 6, dpi = 250, units = 'in')
```
...
#  geom_blank(aes(x =value+36, y = 1), data = x_lims)+
 ...
 # scale_color_manual(values = c('salmon', 'mediumturquoise', 'goldenrod', 'yellow3', 'red', 'slateblue4',  'red', 'dodgerblue')) +
  ...
  ylim(c(-8, 10))
comparison_plot_distance
ggsave(plot = comparison_plot_distance, '../plots/dy_violins.png', width = 12, height = 9, dpi = 250, units = 'in')
```
...
## Compare adjusted fits

```{r}
secondary_data = merged_byyear %>%
  filter(grepl(kind, pattern = 'PCR')) 
