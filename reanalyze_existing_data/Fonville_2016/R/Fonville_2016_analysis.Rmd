...
comparison_plot_distance= merged_byyear %>%
  mutate(lag = ifelse(lag == '<=30d', lag,
                      ifelse(lag == '>90d', lag, 'unknown'))) %>%
  filter(!grepl('ferret', kind)) %>%
  separate(kind, into = c( 'distance type', 'past/future'), sep = ":", remove = F) %>%
  mutate(`past/future` = factor(`past/future`, levels = c(' past strains',' future strains')),
         `distance type` = factor(`distance type`, 
                                  levels = c('ferret distances','child distances', 'vaccine study distances', 'HaNam PCR infection distances'),
                                  labels = c('ferret distances','primary infection', 'secondary vaccination', 'secondary infection'),
                                  ordered = TRUE),
