...
## Plot summary by sample order
```{r}
pcr_summary %>%
  mutate(sample_index = sample_year-infection_time,
         sample_index = factor(sample_index, levels = as.character(-4:4))) %>%
  group_by(sample_index) %>%
  mutate(n_samples = n(),
            n_i = length(unique(subject)))%>%
  ggplot(aes(x = sample_index, y = max_titer)) +
  geom_boxplot() +
  geom_text(aes(x = sample_index, y = 8, label = sprintf('n=%s\n%s subj', n_samples, n_i)))+
  geom_vline(aes(xintercept = 4.5), lty = 2)+
  ylab('max log titer of each individual (jittered)')+
  xlab('sample index (-1=pre-infection; 0=post-infection)')+
  ggtitle('Max titer per individual')
```
...
pcr_summary %>%
  ...
  ungroup() %>%
  group_by(subject) %>%
    select(subject, sample_index, max_titer) %>%
  distinct() %>%
  arrange(sample_index) %>%
  mutate(d_max_titer = dplyr::lag(max_titer)-max_titer) %>%
  ungroup() %>%
  ggplot(aes(x = sample_index, y = d_max_titer)) +
  geom_boxplot() +
  geom_vline(aes(xintercept = 4.5), lty = 2)+
  ylab('max log titer of each individual (jittered)')+
  xlab('sample index (-1=pre-infection; 0=post-infection)')+
  ggtitle('Change in max titer from the last annual sample') 
```
