...
```{r}
  initfun <- function(){
    list(sigma = 1,
         strain_coords = (coords %>% filter(kind == 'antigen') %>% select(starts_with('c')) %>% as.matrix)[-1,],
...
fit_stan_MDS <- function(
  ...
  stopifnot(length(first_strain_coords)==n_dim)
  ...
  model_input_data <- list(
    ...
    first_strain_coords = first_strain_coords
  )
  
  fit <- sampling(
    model, model_input_data, chains = chains, cores = cores, init = initfun,
    iter = 5000,
    # control = list(adapt_delta = 0.3,
    #                max_treedepth = 20),
    ...
  )
}
...
```{r}
fits <- fit_stan_MDS(observed_distances = observed_distmat, n_antigens = 5, n_sera = 5, n_dim = 2, first_strain_coords = c(0,0), chains = 1)
```
